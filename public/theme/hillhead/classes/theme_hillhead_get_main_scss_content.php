 ...
 */
class theme_hillhead_get_main_scss_content {
    ...
    public function __construct($theme) {
        ...
        foreach ($this->sheets as $sheet) {
            $this->compiledcss .= file_get_contents($CFG->dirroot . '/theme/hillhead/scss/'.$sheet.'.scss');
        }

        // ...now append the main scss file style rules...
        $this->compiledcss .= theme_boost_get_main_scss_content($theme);

        $this->sheets = ['hillhead', 'accessibility', 'login'];
        ...
        $this->compiledcss .= file_get_contents($CFG->dirroot . '/theme/hillhead/scss/'.$this->filename);
