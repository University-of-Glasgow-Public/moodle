 ...
 *
 * Based on the settings in /admin/settings.php?section=themesettinghillhead
...
if ($PAGE->pagetype == 'my-index') {
    ...
    if ($dashboardnotification == 'enabled') {
        ...
        $notiftext .= '/theme/hillhead/notification.php?h=' . md5('DashboardNoCourseWarning');
        $notiftext .= '" aria-label="Close"><span aria-hidden="true">&times;</span></a><span class="d-flex-item">';
        $notiftext .= get_config('theme_hillhead', 'hillhead_student_course_alert_text') . '</span></div>';
    }
}
