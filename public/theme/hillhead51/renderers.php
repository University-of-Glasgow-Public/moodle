 ...
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_hillhead51_core_renderer extends core_renderer {

    /**
     * Implementing the render_custom_menu method.
     *
    ...
    protected function render_custom_menu(custom_menu $menu) {
        if (isloggedin()) {
            global $CFG;
            $usesaccessibilitytools = get_user_preferences('theme_hillhead51_accessibility', false);
            $varg = 'clear';
            $spantext = 'Hide';
    ...
    /**
     * Method stub. Called in drawers.php
     */
...
// For the Template Wizard plugin.
global $CFG;
require_once($CFG->dirroot . "/course/renderer.php");
require_once($CFG->dirroot . "/course/classes/management_renderer.php");

/**
 * This core course renderer has been overwritten to inject additional links to
 * /local/template/index.php at all locations where 'Add a new course' text exists.
 *
 ...
 */
class theme_hillhead51_core_course_renderer extends core_course_renderer {
    ...
    public function course_category($category) {
        ...
        $site = get_site();
        $actionbar = new \theme_hillhead51\output\theme_hillhead51_category_action_bar($this->page, $coursecat);
...
/**
 * This core course management renderer has been overwritten to inject additional links
 * to /local/template/index.php at all locations where 'Create new course' text exists.
 *
 ...
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_hillhead51_core_course_management_renderer extends core_course_management_renderer {
     ...
     * @return string
     */
    public function course_listing_actions(core_course_category $category, core_course_list_element|null $course = null,
        $perpage = 20) {
        if (!\theme_hillhead51\theme_hillhead51_exists_template_plugin::template_plugin_exists()) {
        ...
        if ($category->can_create_course()) {
            $url = new moodle_url('/course/edit.php', ['category' => $category->id, 'returnto' => 'catmanage']);
            $actions[] = html_writer::link($url, get_string('createnewcourse'), ['class' => 'btn btn-secondary']);

            $url = new moodle_url('/local/template/index.php', ['category' => $category->id, 'returnto' => 'catmanage']);
            $actions[] = html_writer::link($url, get_string('addnewcourseviatemplate', 'local_template'),
                ['class' => 'btn btn-secondary']);
        }
        if ($category->can_request_course()) {
            // Request a new course.
            $url = new moodle_url('/course/request.php', ['category' => $category->id, 'return' => 'management']);
        ...
        if ($category->can_resort_courses()) {
            ...
            $fullnameurl = new moodle_url($baseurl, ['resort' => 'fullname']);
            $fullnameurldesc = new moodle_url($baseurl, ['resort' => 'fullnamedesc']);
            $shortnameurl = new moodle_url($baseurl, ['resort' => 'shortname']);
            $shortnameurldesc = new moodle_url($baseurl, ['resort' => 'shortnamedesc']);
            $idnumberurl = new moodle_url($baseurl, ['resort' => 'idnumber']);
            $idnumberdescurl = new moodle_url($baseurl, ['resort' => 'idnumberdesc']);
            $timecreatedurl = new moodle_url($baseurl, ['resort' => 'timecreated']);
            $timecreateddescurl = new moodle_url($baseurl, ['resort' => 'timecreateddesc']);
        ...
        $menu = new action_menu([
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 5]), null, 5),
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 10]), null, 10),
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 20]), null, 20),
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 50]), null, 50),
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 100]), null, 100),
            new action_menu_link_secondary(new moodle_url($this->page->url, ['perpage' => 999]), null, $strall),
        ]);
        if ((int)$perpage === 999) {
            $perpage = $strall;
        }
