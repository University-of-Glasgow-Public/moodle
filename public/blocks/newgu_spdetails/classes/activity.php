...
class activity {
    ...
    public static function get_activityitems(int $subcategoryid, int $userid, string $activetab): array {
        ...
        if ($getactivities[1] !== '') {
            // An error has occurred. More than likely a course setup issue. What do we do with the error message that's returned?
            $error = true;
            $errormsg = strip_tags($getactivities[1]);
        } else {
        ...
        $coursedata['error'] = $error;
        $coursedata['errormsg'] = $errormsg;
